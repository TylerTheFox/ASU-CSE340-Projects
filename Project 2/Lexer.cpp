...
void Lexer::processTokenInput(const std::vector<std::string>& stringVect, const std::map<size_t, unsigned long long> & lineMapPreProcess)
{
    ...
    for (const auto & word : stringVect)
    {
        ...
        {
            std::shared_ptr<TokenID> data(new TokenID);
            data->setLineNum(sortOutLineNumbers());
            tokens.push_back(data);
            continue;
        }
        ...
        {
            std::shared_ptr<TokenOperatorDef> data(new TokenOperatorDef);
            data->setLineNum(sortOutLineNumbers());
            tokens.push_back(data);
            continue;
        }
        ...
        {
            std::shared_ptr<TokenHash> data(new TokenHash);
            data->setLineNum(sortOutLineNumbers());
            tokens.push_back(data);
            continue;
        }
        ...
        {
            std::shared_ptr<TokenDoubleHash> data(new TokenDoubleHash);
            data->setLineNum(sortOutLineNumbers());
            tokens.push_back(data);
            continue;
        }
